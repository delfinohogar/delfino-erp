...
| MIGRADOR_BASELINE | `--marcar-aplicadas` sobre una base vacía, y sobre una base con las repetibles reales ya desplegadas | sobre la vacía **aborta** con exit != 0 y no marca **nada** (ni numeradas ni repetibles), porque `repetibles/crear_venta.sql` no está desplegada (R37); sobre la que sí está, marca sin ejecutar y nunca se dispara solo en una corrida normal | TASK-001, endurecido en TASK-018 |
...
| MIGRADOR_REPETIBLES_ATOMICIDAD | una repetible de `db/repetibles/` falla en su segunda sentencia | NO queda registrada en `schema_repetibles`, no deja efectos, las posteriores no se aplican y el reintento la vuelve a intentar (sale != 0 otra vez, no "sin cambios") | TASK-012 accept |
...
| MIGRADOR_REPETIBLES_DIRECTORIO | `db/repetibles/` no existe, y `db/repetibles/` vacío; más el **control** de que el directorio que arma el test es el que resuelve `migrar.js` | el migrador sale 0 en los dos casos, informa "Repetibles: sin cambios" y crea `schema_repetibles` vacía. El control existe porque los dos primeros casos dan verde **también** si el test deja las repetibles en un directorio que el migrador no mira: pasó en TASK-018 con el renombre `functions/` → `repetibles/` | TASK-012 accept, control agregado en TASK-018 |
...
| MIGRADOR_REPETIBLES_CONVENCIONES | `--marcar-aplicadas` con una repetible que la base no tiene; la misma tras un `DROP FUNCTION` a mano; una repetible borrada del disco | el baseline **falla** con exit != 0, nombra archivo y función, y **no escribe ni una fila** en ninguna de las dos tablas de control; el `DROP FUNCTION` lo dispara igual, porque el chequeo recorre todas las repetibles y no solo las pendientes; la borrada del disco deja fila huérfana que `--estado` reporta y el migrador no falla ni borra la función | TASK-012 + R37 cerrado en TASK-018 |
| CREAR_VENTA_CANONICA | qué definición de `crear_venta()` corre en la suite después de `recrearEsquema()` | `prosrc` es, byte a byte, el cuerpo de `backend/db/repetibles/crear_venta.sql` normalizado a LF, y `pg_get_functiondef()` no contiene un solo `\r`; aplicar **solo** las numeradas deja en cambio la copia histórica de `0004` (163 `\r` en este checkout), o sea que la comparación discrimina de verdad; y el `COMMENT` de la migración `0006` apunta a `repetibles/` | TASK-018 accept |
